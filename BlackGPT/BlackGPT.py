        ...
        with gr.Column():
            with gr.Column(min_width=50,scale=1):
                ...
                with gr.Tab(label="ChatGPT"):
                    ...
                    model_select_dropdown = gr.Dropdown(label="Select model", choices=MODELS, multiselect=False, value=MODELS[0])
                    with gr.Accordion("parameter", open=False):
                        top_p = gr.Slider(minimum=-0, maximum=1.0, value=1.0, step=0.05,
                                        interactive=True, label="Top-p (nucleus sampling)",)
                        temperature = gr.Slider(minimum=-0, maximum=2.0, value=1.0,
                                                step=0.1, interactive=True, label="Temperature",)
                    use_streaming_checkbox = gr.Checkbox(label="Real-time Transmission of Answers", value=True, visible=enable_streaming_option)
                        ...
                        with gr.Column():
                            with gr.Row():
                                with gr.Column(scale=6):
                                    templateFileSelectDropdown = gr.Dropdown(label="Select Prompt", choices=get_template_names(plain=True), multiselect=False, value=get_template_names(plain=True)[0])
...
logging.info(colorama.Back.GREEN + "\nBlack Bots's warm reminder: visit http://localhost:7860 to view the interface" + colorama.Style.RESET_ALL)
